// Generated by Sichem at 1/23/2018 6:42:34 PM
...
namespace StbSharp
{
	unsafe partial class StbTrueType
	{
		...
		public static stbtt__buf stbtt__cid_get_glyph_subrs(stbtt_fontinfo* info, int glyph_index)
		{
			...
			return
				(stbtt__buf)
					(stbtt__get_subrs((stbtt__buf) (info->cff),
			...
			int* leftSideBearing)
		{
			...
			{
				if ((advanceWidth) != null) *advanceWidth = (int) (ttSHORT(info->data + info->hmtx + 4*(numOfLongHorMetrics - 1)));
		...
		public static int stbtt_GetCodepointKernAdvance(stbtt_fontinfo* info, int ch1, int ch2)
		{
			...
			return
				(int)
					(stbtt_GetGlyphKernAdvance(info, (int) (stbtt_FindGlyphIndex(info, (int) (ch1))),
			...
			else
			{
				if ((hh->num_remaining_in_head_chunk) == (0))
				{
					...
					stbtt__hheap_chunk* c =
						(stbtt__hheap_chunk*) (CRuntime.malloc((ulong) (sizeof (stbtt__hheap_chunk) + +size*count)));
				...
				return (sbyte*) (hh->head) + size*hh->num_remaining_in_head_chunk;
			}
		...
		public static stbtt__active_edge* stbtt__new_active(stbtt__hheap* hh, stbtt__edge* e, int off_x, float start_point,
			void* userdata)
		{
			stbtt__active_edge* z = (stbtt__active_edge*) (stbtt__hheap_alloc(hh, (ulong) (sizeof ((*z))),
			userdata))
			;
			...
			int off_x, int off_y, void* userdata)
		{
			...
			while ((j) < (result->h))
			{
				...
				CRuntime.memset(scanline, (int) (0), (ulong) (result->w*sizeof ((scanline[0]))))
				;
				CRuntime.memset(scanline2, (int) (0), (ulong) ((result->w + 1)*sizeof ((scanline[0]))))
				;
				while ((*step) != null)
				{
		...
		public static void stbtt__rasterize(stbtt__bitmap* result, stbtt__point* pts, int* wcount, int windings, float scale_x,
			float scale_y, float shift_x, float shift_y, int off_x, int off_y, int invert, void* userdata)
		{
			...
			e = (stbtt__edge*) (CRuntime.malloc((ulong) (sizeof ((*e))*(n + 1))))
			;
			if ((e) == (0)) return;
			n = (int) (0);
			m = (int) (0);
		...
		public static int stbtt__tesselate_curve(stbtt__point* points, int* num_points, float x0, float y0, float x1, float y1,
			...
			float y1, float x2, float y2, float x3, float y3, float objspace_flatness_squared, int n)
		{
			...
			float longlen =
				(float)
					(CRuntime.sqrt((double) (dx0*dx0 + dy0*dy0)) + CRuntime.sqrt((double) (dx1*dx1 + dy1*dy1)) +
			...
			int** contour_lengths, int* num_contours, void* userdata)
		{
			...
			*num_contours = (int) (n);
			if ((n) == (0)) return 0;
			*contour_lengths = (int*) (CRuntime.malloc((ulong) (sizeof ((**contour_lengths))*n)))
			;
			...
			for (pass = (int) (0); (pass) < (2); ++pass)
			{
				...
				if ((pass) == (1))
				{
					points = (stbtt__point*) (CRuntime.malloc((ulong) (num_points*sizeof ((points[0])))))
					;
		...
		public static byte* stbtt_GetCodepointBitmapSubpixel(stbtt_fontinfo* info, float scale_x, float scale_y, float shift_x,
			float shift_y, int codepoint, int* width, int* height, int* xoff, int* yoff)
		{
			...
			int ph, int first_char, int num_chars, stbtt_bakedchar* chardata)
		{
			...
			memset(pixels, (int) (0), (ulong) (pw*ph));
			x = (int) (y = (int) (1));
			bottom_y = (int) (1);
		...
		{
			stbrp_context* context = (stbrp_context*) (CRuntime.malloc((ulong) (sizeof ((*context)))))
			;
			int num_nodes = (int) (pw - padding);
			stbrp_node* nodes = (stbrp_node*) (CRuntime.malloc((ulong) (sizeof ((*nodes))*num_nodes)))
			;
			...
			stbtt_pack_range* ranges, int num_ranges, stbrp_rect* rects)
		{
			...
			for (i = (int) (0); (i) < (num_ranges); ++i)
			{
				...
				float scale =
					(float)
						((fh) > (0) ? stbtt_ScaleForPixelHeight(info, (float) (fh)) : stbtt_ScaleForMappingEmToPixels(info, (float) (-fh)));
			...
			for (i = (int) (0); (i) < (num_ranges); ++i)
			{
				...
				for (j = (int) (0); (j) < (ranges[i].num_chars); ++j)
				{
					...
					if ((r->was_packed) != 0)
					{
						...
						stbtt_GetGlyphBitmapBox(info, (int) (glyph), (float) (scale*spc->h_oversample), (float) (scale*spc->v_oversample),
							&x0, &y0, &x1, &y1);
						stbtt_MakeGlyphBitmapSubpixel(info, spc->pixels + r->x + r->y*spc->stride_in_bytes,
							...
							(float) (scale*spc->h_oversample), (float) (scale*spc->v_oversample), (float) (0), (float) (0), (int) (glyph));
			...
			stbtt_pack_range* ranges, int num_ranges)
		{
			...
			rects = (stbrp_rect*) (CRuntime.malloc((ulong) (sizeof ((*rects))*n)))
			;
		...
		public static int stbtt__ray_intersect_bezier(float* orig, float* ray, float* q0, float* q1, float* q2, float** hits)
		{
			...
			else
			{
				...
				hits[0][0] = (float) (q0rd + s0*(2.0f - 2.0f*s0)*q10d + s0*s0*q20d);
				hits[0][1] = (float) (a*s0 + b);
				if ((num_s) > (1))
				{
					hits[1][0] = (float) (q0rd + s1*(2.0f - 2.0f*s1)*q10d + s1*s1*q20d);
					hits[1][1] = (float) (a*s1 + b);
					return (int) (2);
				}
				else
				{
		...
		public static int stbtt__compute_crossings_x(float x, float y, int nverts, stbtt_vertex* verts)
		{
			...
			orig[0] = (float) (x);
			orig[1] = (float) (y);
			y_frac = ((float) (fmod((double) (y), (double) (1.0f))));
				...
				if ((verts[i].type) == (STBTT_vcurve))
				{
					...
					if ((((y) > (ay)) && ((y) < (by))) && ((x) > (ax)))
					{
						...
						else
						{
							...
							if ((num_hits) >= (1)) if ((hits[0][0]) < (0)) winding += (int) ((hits[0][1]) < (0) ? -1 : 1);
							if ((num_hits) >= (2)) if ((hits[1][0]) < (0)) winding += (int) ((hits[1][1]) < (0) ? -1 : 1);
						}
					}
				}
			}
		...
		public static int stbtt__solve_cubic(float a, float b, float c, float* r)
		{
			...
			else
			{
				...
				float v = (float) ((float) (acos((double) (-sqrt((double) (-27/p3))*q/2)))/3);
			...
			float pixel_dist_scale, int* width, int* height, int* xoff, int* yoff)
		{
			...
			stbtt_GetGlyphBitmapBoxSubpixel(info, (int) (glyph), (float) (scale), (float) (scale), (float) (0.0f), (float) (0.0f),
				&ix0, &iy0, &ix1, &iy1);
					...
					for (x = (int) (ix0); (x) < (ix1); ++x)
					{
						...
						for (i = (int) (0); (i) < (num_verts); ++i)
						{
							...
							if ((verts[i].type) == (STBTT_vline))
							{
								...
								float dist =
									(float) ((float) (CRuntime.fabs((double) ((x1 - x0)*(y0 - sy) - (y1 - y0)*(x0 - sx))))*precompute[i]);
								if ((dist) < (min_dist))
								{
		...
		public static int stbtt_CompareUTF8toUTF16_bigendian_internal(sbyte* s1, int len1, sbyte* s2, int len2)
		{
			return
				(int)
					((len1) == (stbtt__CompareUTF8toUTF16_bigendian_prefix((byte*) (s1), (int) (len1), (byte*) (s2), (int) (len2)))
						? 1
						: 0);
		}
						...
						if ((matchlen) >= (0))
						{
							    ...
							    ((ttUSHORT(fc + loc + 12 + 4)) == (language)))
							{
								...
								else if (((matchlen) < (nlen)) && ((name[matchlen]) == (' ')))
								{
									++matchlen;
									if (
										(stbtt_CompareUTF8toUTF16_bigendian_internal((sbyte*) (name + matchlen), (int) (nlen - matchlen),
		...
		{
			int nlen = (int) (strlen((sbyte*) (name)));
			uint nm;
			uint hd;
			...
			int first_char, int num_chars, stbtt_bakedchar* chardata)
		{
			return
				(int)
					(stbtt_BakeFontBitmap_internal(data, (int) (offset), (float) (pixel_height), pixels, (int) (pw), (int) (ph),
						(int) (first_char), (int) (num_chars), chardata));
		}
