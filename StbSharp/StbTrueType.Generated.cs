	...
	unsafe partial class StbTrueType
	{
			...
			int* leftSideBearing)
		{
			...
			else
			{
				...
				if ((leftSideBearing) != null)
					*leftSideBearing =
						(int) (ttSHORT(info->data + info->hmtx + 4 * numOfLongHorMetrics +
					...
					switch (vertices[i].type)
					{
						...
						case STBTT_vcubic:
							stbtt__tesselate_cubic(points, &num_points, (float) (x), (float) (y),
								(float) (vertices[i].cx),
								(float) (vertices[i].cy), (float) (vertices[i].cx1), (float) (vertices[i].cy1),
								(float) (vertices[i].x),
		...
		public static void stbtt_MakeGlyphBitmapSubpixelPrefilter(stbtt_fontinfo* info, byte* output, int out_w,
			int out_h,
			int out_stride, float scale_x, float scale_y, float shift_x, float shift_y, int prefilter_x,
			int prefilter_y,
			float* sub_x, float* sub_y, int glyph)
		{
			stbtt_MakeGlyphBitmapSubpixel(info, output, (int) (out_w - (prefilter_x - 1)),
				(int) (out_h - (prefilter_y - 1)),
				...
				for (j = (int) (0); (j) < (ranges[i].num_chars); ++j)
				{
					...
					if ((r->was_packed) != 0)
					{
						...
						stbtt_MakeGlyphBitmapSubpixel(info, spc->pixels + r->x + r->y * spc->stride_in_bytes,
							...
							(float) (scale * spc->h_oversample), (float) (scale * spc->v_oversample), (float) (0),
							(float) (0),
							(int) (glyph));
			...
			scale_y = (float) (-scale_y);
			{
				...
				for (y = (int) (iy0); (y) < (iy1); ++y)
				{
					for (x = (int) (ix0); (x) < (ix1); ++x)
					{
						...
						int winding =
							(int) (stbtt__compute_crossings_x((float) (x_gspace), (float) (y_gspace), (int) (num_verts),
								verts));
						for (i = (int) (0); (i) < (num_verts); ++i)
						{
							float x0 = (float) (verts[i].x * scale_x);
			...
			for (i = (int) (0); (i) < (count); ++i)
			{
				...
				if ((id) == (target_id))
				{
					    ...
					    (((platform) == (3)) && ((encoding) == (10))))
					{
						...
						int matchlen =
							(int) (stbtt__CompareUTF8toUTF16_bigendian_prefix(name, (int) (nlen),
