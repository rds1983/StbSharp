...
namespace StbSharp
{
	unsafe partial class StbVorbis
	{
		...
		public static int start_page_no_capturepattern(stb_vorbis f)
		{
			...
			uint n;
			if (0 != get8(f))
				return (int)(error(f, (int)(VORBIS_invalid_stream_structure_version)));
		...
		public static int start_page(stb_vorbis f)
		{
			if (capture_pattern(f) == 0)
				return (int)(error(f, (int)(VORBIS_missing_capture_pattern)));
					...
					if ((pass) == (0))
					{
						for (j = (int)(0); (j) < (ch); ++j)
						{
							if (do_not_decode[j] == 0)
							{
								...
								if ((c2->sparse) != 0)
									temp = (int)(c2->sorted_values[temp]);
								if ((temp) == (-1))
									goto done;
			...
			int* p_right_end, int* mode)
		{
			...
			{
				if (((f).push_mode) != 0)
					return (int)(error(f, (int)(VORBIS_bad_packet_type)));
		...
		public static int is_whole_packet_present(stb_vorbis f, int end_page)
		{
			...
			if (s != -1)
			{
				...
				if ((s) == (f.segment_count))
					s = (int)(-1);
				if ((p) > (f.stream_end))
					return (int)(error(f, (int)(VORBIS_need_more_data)));
				first = (int)(0);
			}
		...
		public static int start_decoder(stb_vorbis f)
		{
			...
			if (start_page(f) == 0)
				return (int)(0);
			if ((f.page_flag & 2) == 0)
				return (int)(error(f, (int)(VORBIS_invalid_first_page)));
			if ((f.page_flag & 4) != 0)
				return (int)(error(f, (int)(VORBIS_invalid_first_page)));
			if ((f.page_flag & 1) != 0)
				return (int)(error(f, (int)(VORBIS_invalid_first_page)));
			...
			if (f.channels == 0)
				return (int)(error(f, (int)(VORBIS_invalid_first_page)));
			if ((f.channels) > (16))
				return (int)(error(f, (int)(VORBIS_too_many_channels)));
			...
			if (((f).push_mode) != 0)
			{
				if (is_whole_packet_present(f, (int)(1)) == 0)
				{
					if ((f.error) == (VORBIS_invalid_stream))
						f.error = (int)(VORBIS_invalid_setup);
					return (int)(0);
				}
			}
			...
			stb_vorbis_alloc* alloc)
		{
			...
			{
				if ((p.eof) != 0)
					*error = (int)(VORBIS_need_more_data);
				else
					*error = (int)(p.error);
				return (null);
			}
		...
		public static int seek_to_sample_coarse(stb_vorbis f, uint sample_number)
		{
			...
			if ((stream_length) == (0))
				return (int)(error(f, (int)(VORBIS_seek_without_length)));
			if ((sample_number) > (stream_length))
				return (int)(error(f, (int)(VORBIS_seek_invalid)));
		...
		public static uint stb_vorbis_stream_length_in_samples(stb_vorbis f)
		{
			...
			uint last_page_loc;
			if (((f).push_mode) != 0)
				return (uint)(error(f, (int)(VORBIS_invalid_api_mixing)));
